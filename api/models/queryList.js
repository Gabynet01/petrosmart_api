...
let driverQueries = {
    ...
    "vehicleById": "select * from petrosmart_vehicles where veh_id = ?",
    ...
    "fetchAllTransactionsByVehicleId": "SELECT PurchaseHistory.*, S.Name AS StationName, S.address AS StationAddress, V.name as VehicleName, V.number_plate AS VehicleRegNo FROM petrosmart_api_purchase_info AS PurchaseHistory INNER JOIN petrosmart_fuel_station AS S ON PurchaseHistory.station = S.station_id INNER JOIN petrosmart_vehicles AS V ON PurchaseHistory.vehicle = V.veh_id where veh_id = ? AND (payment_code_status = 'INACTIVE' OR payment_code_status = 'ACTIVE') ORDER BY created_at DESC",

    "fetchAllTransactionsByStationId": "SELECT PurchaseHistory.*, S.Name AS StationName, S.address AS StationAddress, V.name as VehicleName, V.number_plate AS VehicleRegNo FROM petrosmart_api_purchase_info AS PurchaseHistory INNER JOIN petrosmart_fuel_station AS S ON PurchaseHistory.station = S.station_id INNER JOIN petrosmart_vehicles AS V ON PurchaseHistory.vehicle = V.veh_id where station_id = ? AND (payment_code_status = 'INACTIVE' OR payment_code_status = 'ACTIVE') ORDER BY created_at DESC",
    ...
    "fetchTransactionsByDayMonthYear": "SELECT PurchaseHistory.*, S.Name AS StationName, S.address AS StationAddress, V.name as VehicleName, V.number_plate AS VehicleRegNo FROM petrosmart_api_purchase_info AS PurchaseHistory INNER JOIN petrosmart_fuel_station AS S ON PurchaseHistory.station = S.station_id INNER JOIN petrosmart_vehicles AS V ON PurchaseHistory.vehicle = V.veh_id WHERE user = ? AND DAY(PurchaseHistory.created_at) = ? AND MONTH(PurchaseHistory.created_at) = ? AND YEAR(PurchaseHistory.created_at) = ? AND (payment_code_status = 'INACTIVE' OR payment_code_status = 'ACTIVE') ORDER BY created_at DESC",

    "fetchVehicleTransactionsByDayMonthYear": "SELECT PurchaseHistory.*, S.Name AS StationName, S.address AS StationAddress, V.name as VehicleName, V.number_plate AS VehicleRegNo FROM petrosmart_api_purchase_info AS PurchaseHistory INNER JOIN petrosmart_fuel_station AS S ON PurchaseHistory.station = S.station_id INNER JOIN petrosmart_vehicles AS V ON PurchaseHistory.vehicle = V.veh_id WHERE veh_id = ? AND DAY(PurchaseHistory.created_at) = ? AND MONTH(PurchaseHistory.created_at) = ? AND YEAR(PurchaseHistory.created_at) = ? AND (payment_code_status = 'INACTIVE' OR payment_code_status = 'ACTIVE') ORDER BY created_at DESC",

    "fetchStationTransactionsByDayMonthYear": "SELECT PurchaseHistory.*, S.Name AS StationName, S.address AS StationAddress, V.name as VehicleName, V.number_plate AS VehicleRegNo FROM petrosmart_api_purchase_info AS PurchaseHistory INNER JOIN petrosmart_fuel_station AS S ON PurchaseHistory.station = S.station_id INNER JOIN petrosmart_vehicles AS V ON PurchaseHistory.vehicle = V.veh_id WHERE station_id = ? AND DAY(PurchaseHistory.created_at) = ? AND MONTH(PurchaseHistory.created_at) = ? AND YEAR(PurchaseHistory.created_at) = ? AND (payment_code_status = 'INACTIVE' OR payment_code_status = 'ACTIVE') ORDER BY created_at DESC",
...
let stationQueries = {
    "listStations": "SELECT FS.*, POS.* FROM petrosmart_fuel_station AS FS INNER JOIN petrosmart_pos AS POS ON POS.pos_id in (FS.pos_selected)",
...
let fleetManagerQueries = {
    ...
    "getAllRequests": "SELECT FM.*, V.name AS VehicleName, V.Id AS VehicleId, V.model AS VehicleModel, V.fuel_type AS FuelType, C.full_name AS CompanyName, CW.wallet_num AS CompanyWallet, D.name AS DriverName, D.mob as DriverMobile, S.name AS StationName, S.address AS StationAddress, SW.wallet_num AS StationWallet FROM petrosmart_fleet_manager_requests AS FM INNER JOIN petrosmart_customer AS C ON FM.company_id = C.cust_id INNER JOIN petrosmart_customer_wallet AS CW ON FM.company_wallet_id = CW.custw_id INNER JOIN petrosmart_drivers AS D ON FM.driver_id = D.driver_id INNER JOIN petrosmart_fuel_station AS S ON FM.station_id = S.station_id INNER JOIN petrosmart_fuelstation_wallet AS SW ON FM.station_wallet_id = SW.custw_id INNER JOIN petrosmart_vehicles AS V ON FM.driver_id in (V.drivers_selected) WHERE fleet_manager_id = ? AND YEAR(FM.created_at) = ? ORDER BY created_at DESC",

    "getRequestsByStatus": "SELECT FM.*, V.name AS VehicleName, V.Id AS VehicleId, V.model AS VehicleModel, V.fuel_type AS FuelType, C.full_name AS CompanyName, CW.wallet_num AS CompanyWallet, D.name AS DriverName, D.mob as DriverMobile, S.name AS StationName, S.address AS StationAddress, SW.wallet_num AS StationWallet FROM petrosmart_fleet_manager_requests AS FM INNER JOIN petrosmart_customer AS C ON FM.company_id = C.cust_id INNER JOIN petrosmart_customer_wallet AS CW ON FM.company_wallet_id = CW.custw_id INNER JOIN petrosmart_drivers AS D ON FM.driver_id = D.driver_id INNER JOIN petrosmart_fuel_station AS S ON FM.station_id = S.station_id INNER JOIN petrosmart_fuelstation_wallet AS SW ON FM.station_wallet_id = SW.custw_id INNER JOIN petrosmart_vehicles AS V ON FM.driver_id in (V.drivers_selected) WHERE fleet_manager_id = ? AND YEAR(FM.created_at) = ? AND approval_flag = ? ORDER BY created_at DESC",

    "fleetManagerByPhoneNumber": "select * from petrosmart_fleet_managers where mobile_number = ? ",

    "getAssignedDrivers": "select drivers_selected from petrosmart_fleet_managers where user_id = ? ",

    "transactionsByYearChart": "SELECT SUM(amount) as total_amount, month(created_at) as month, year(created_at) as year FROM petrosmart_fleet_manager_requests where fleet_manager_id = ? AND YEAR(created_at) = ? AND approval_flag = 'APPROVED' group by YEAR(created_at), MONTH(created_at)",

    "transactionsByMonthChart": "SELECT SUM(amount) AS total_amount, DAYOFMONTH(created_at) AS day, year(created_at) as year FROM petrosmart_fleet_manager_requests where fleet_manager_id = ? AND MONTH(created_at) = ? AND YEAR(created_at) = ? AND approval_flag = 'APPROVED' GROUP BY DAYOFMONTH(created_at)",

    "transactionsByWeekChart": "SELECT SUM(amount) AS total_amount, DAYNAME(created_at) AS day_name, year(created_at) as year FROM petrosmart_fleet_manager_requests where fleet_manager_id = ? AND YEARWEEK(created_at) = YEARWEEK(NOW()) AND approval_flag = 'APPROVED' GROUP BY DAYOFMONTH(created_at)",

    "fetchRequestsByDayMonthYear": "SELECT FM.*, V.name AS VehicleName, V.Id AS VehicleId, V.model AS VehicleModel, V.fuel_type AS FuelType, C.full_name AS CompanyName, CW.wallet_num AS CompanyWallet, D.name AS DriverName, D.mob as DriverMobile, S.name AS StationName, S.address AS StationAddress, SW.wallet_num AS StationWallet FROM petrosmart_fleet_manager_requests AS FM INNER JOIN petrosmart_customer AS C ON FM.company_id = C.cust_id INNER JOIN petrosmart_customer_wallet AS CW ON FM.company_wallet_id = CW.custw_id INNER JOIN petrosmart_drivers AS D ON FM.driver_id = D.driver_id INNER JOIN petrosmart_fuel_station AS S ON FM.station_id = S.station_id INNER JOIN petrosmart_fuelstation_wallet AS SW ON FM.station_wallet_id = SW.custw_id INNER JOIN petrosmart_vehicles AS V ON FM.driver_id in (V.drivers_selected) WHERE fleet_manager_id = ? AND DAY(FM.created_at) = ? AND MONTH(FM.created_at) = ? AND YEAR(FM.created_at) = ? ORDER BY created_at DESC",

    "fetchRequestsByMonthYear": "SELECT FM.*, V.name AS VehicleName, V.Id AS VehicleId, V.model AS VehicleModel, V.fuel_type AS FuelType, C.full_name AS CompanyName, CW.wallet_num AS CompanyWallet, D.name AS DriverName, D.mob as DriverMobile, S.name AS StationName, S.address AS StationAddress, SW.wallet_num AS StationWallet FROM petrosmart_fleet_manager_requests AS FM INNER JOIN petrosmart_customer AS C ON FM.company_id = C.cust_id INNER JOIN petrosmart_customer_wallet AS CW ON FM.company_wallet_id = CW.custw_id INNER JOIN petrosmart_drivers AS D ON FM.driver_id = D.driver_id INNER JOIN petrosmart_fuel_station AS S ON FM.station_id = S.station_id INNER JOIN petrosmart_fuelstation_wallet AS SW ON FM.station_wallet_id = SW.custw_id INNER JOIN petrosmart_vehicles AS V ON FM.driver_id in (V.drivers_selected) WHERE fleet_manager_id = ? AND MONTH(FM.created_at) = ? AND YEAR(FM.created_at) = ? ORDER BY created_at DESC",

    "fetchRequestsByYear": "SELECT FM.*, V.name AS VehicleName, V.Id AS VehicleId, V.model AS VehicleModel, V.fuel_type AS FuelType, C.full_name AS CompanyName, CW.wallet_num AS CompanyWallet, D.name AS DriverName, D.mob as DriverMobile, S.name AS StationName, S.address AS StationAddress, SW.wallet_num AS StationWallet FROM petrosmart_fleet_manager_requests AS FM INNER JOIN petrosmart_customer AS C ON FM.company_id = C.cust_id INNER JOIN petrosmart_customer_wallet AS CW ON FM.company_wallet_id = CW.custw_id INNER JOIN petrosmart_drivers AS D ON FM.driver_id = D.driver_id INNER JOIN petrosmart_fuel_station AS S ON FM.station_id = S.station_id INNER JOIN petrosmart_fuelstation_wallet AS SW ON FM.station_wallet_id = SW.custw_id INNER JOIN petrosmart_vehicles AS V ON FM.driver_id in (V.drivers_selected) WHERE fleet_manager_id = ? AND YEAR(FM.created_at) = ? ORDER BY created_at DESC",
